<?php

require_once dirname(__DIR__).'/src/Config.php';
require_once dirname(__DIR__).'/src/ToolLogin.php';
require_once dirname(__DIR__).'/src/ToolCrawl.php';
require_once dirname(__DIR__).'/src/Timetable.php';

class TimetableTest extends PHPUnit_Framework_TestCase
{
    public function testLoginAndReturnCookie()
    {
        $toolLogin = new \Xu42\Qznjw2014\ToolLogin('1305040201', '2905ryoma');
        $this->assertStringStartsWith('JSESSIONID=', $cookie = $toolLogin->getCookie());
        return $cookie;
    }

    /**
     * @depends testLoginAndReturnCookie
     */
    public function testGetTimetableWithSemester($cookie)
    {
        $timetable = new \Xu42\Qznjw2014\Timetable($cookie);
        $info = @$timetable->get('2015-2016-2');
        $this->assertInternalType('array', $info);
    }

    /**
     * @depends testLoginAndReturnCookie
     */
    public function testGetTimetableWithWeek($cookie)
    {
        $timetable = new \Xu42\Qznjw2014\Timetable($cookie);
        $info = @$timetable->get('2015-2016-2', '5');
        $this->assertInternalType('array', $info);
    }

}
